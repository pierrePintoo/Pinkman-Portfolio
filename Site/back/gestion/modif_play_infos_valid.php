...
}; //sous categorie/*$vid_news = $_POST['video-accueil'];*/
...
if (isset($_POST['video-titre']) && $_POST['video-titre'] != "") { //TITRE
    ...
    $req = 'UPDATE playlist SET play_titre = "'.$titre_video.'" WHERE play_id = '. $id_video .';';
    $res =$db->query($req);
}
...
if (isset($_POST['video-desc']) && $_POST['video-desc'] != "") { //DESCRIPTION
    ...
    $req = 'UPDATE playlist SET play_desc = "'.$desc_video.'" WHERE play_id = '. $id_video .';';
    $res =$db->query($req);
}
...
//MODIFICATION DE LA VIDEO EN QUESTION
            $requete = 'UPDATE playlist SET play_minia = "'.$fichier.'" WHERE play_id = '. $id_video .';';
...
$req = 'UPDATE playlist SET play_categorie = "'.$categorie_video.'" , play_souscat = "'.$sous_cat_video.'"  WHERE play_id = '. $id_video .';';
$res =$db->query($req);
header('location:playlist.php');
?>

    <h2 class="text-center">PLaylist modifiée avec succès !</h2>
